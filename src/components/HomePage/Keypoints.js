...
const OrbitalTrail = () => {
  ...
  // Expanded data with images and descriptions (matching original homeAbout structure)
  const hexagonData = [
    {
      id: 1,
  ...
  // Check if orb is near a hexagon - only on client side
  useEffect(() => {
    if (!isMounted) return;
    
    hexagonData.forEach((hex, index) => {
      const hexPos = calculateHexPosition(index, hexagonData.length);
      const orbAngle = rotation;
      const angleDiff = Math.abs((hexPos.angle - orbAngle + 360) % 360);
  ...
  // Don't render the animation during SSR to prevent hydration mismatch
  if (!isMounted) {
    return (
      <div className="w-full bg-slate-900 flex items-center justify-center overflow-x-hidden">
        <div className="w-full max-w-[1200px] px-4 min-h-[800px] flex flex-col items-center">
          <h2 
            className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-3 mt-8 px-4"
            style={{
              fontWeight: 700,
              letterSpacing: '2px',
              textShadow: `
    ...
    <>
      <div className="w-full flex items-center justify-center overflow-x-hidden">
        <div className="w-full max-w-[1200px] px-4 py-8 flex flex-col items-center">
          {/* Added Title: Why Choose Connecting Dots ERP */}
          <h2 
            className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-3 mt-4 md:mt-3 px-4"
            style={{
              fontWeight: 700,
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between w-full gap-8">
            ...
            <div className={`relative ${isMobile ? 'w-72 h-72' : 'w-96 h-96'} flex items-center justify-center`}>
              ...
              {/* Hexagonal shapes around the circle */}
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <React.Fragment>
                        <div className={`absolute inset-0 flex items-center justify-center text-center ${isMobile ? 'text-[8px] p-1' : 'text-[13px] p-3'} ${isActive ? 'text-white' : 'text-white'}`}>
                          {hex.label}
                        </div>
                      </React.Fragment>
                    </div>
              ...
              {/* Orbital path (optional visual guide) */}
            ...
            {/* Right side: Text and icons - Grid layout for mobile */}
                ...
                <div 
                  ...
                  >
                    {isMobile ? (
                      ...
                      // Desktop layout: Logo on left, text on right
                      <>
