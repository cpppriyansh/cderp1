...
import { FaStar, FaUser, FaEnvelope, FaPhone, FaMapMarkerAlt } from "react-icons/fa";
...
const initialCourses = [
  { id: "1", name: "SAP S/4 HANA Courses", initialCount: 12, count: 12, icon: "/Icons/sap (3).avif", startDays: 1, rating: 4.9 },
  { id: "2", name: "Masters in Data Science", initialCount: 7, count: 7, icon: "/Icons/MnDS.avif", startDays: 2, rating: 4.9 },
  { id: "3", name: "SAP FICO Course", initialCount: 9, count: 9, icon: "/Icons/online-analytical.avif", startDays: 2, rating: 4.7 },
  { id: "4", name: "SAP MM Course", initialCount: 8, count: 8, icon: "/Icons/sap (3).avif", startDays: 2, rating: 4.6 },
  { id: "5", name: "SAP ABAP Course", initialCount: 8, count: 8, icon: "/Icons/business-intelligence.avif", startDays: 1, rating: 4.9 },
  { id: "6", name: "SAP HR/HCM Course", initialCount: 12, count: 12, icon: "/Icons/sap (3).avif", startDays: 4, rating: 4.5 },
  { id: "7", name: "HR Course", initialCount: 15, count: 15, icon: "/Icons/HR PR2.avif", startDays: 3, rating: 4.6 },
  { id: "8", name: "SAP SD Course", initialCount: 13, count: 13, icon: "/Icons/sap.png", startDays: 2, rating: 4.5 },
  { id: "9", name: "SAP SUCCESSFACTOR Course", initialCount: 17, count: 17, icon: "/Icons/cloud-data.avif", startDays: 5, rating: 4.6 },
];
...
const Courses = () => {
  ...
  useEffect(() => {
    ...
    const initializedCourses = courses.map((course) => {
      ...
      const adjustedCount = Math.max(2, Math.floor(course.initialCount * timeRatio * 0.8 + course.initialCount * 0.2));
    ...
    initializedCourses.forEach((course, index) => {
      const timerId = setInterval(() => updateCourseTimer(index), 1000);
  ...
  useEffect(() => {
    document.body.classList.toggle("modal-open", showPopupForm);
  ...
  const updateCourseTimer = (index) => {
    setCourses((prevCourses) => {
      ...
      if (diffMs <= 0) {
        ...
        if (Math.random() < 0.0001 && course.count > 2) course.count -= 1;
      }
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < emptyStars; i++) {
      stars.push(<FaStar key={`empty-star-${i}`} className={`${styles.starIcon} ${styles.emptyStar}`} />);
    }
  ...
  return (
    <div id="popCourses" className={`${styles.coursesContainer} text-center`}>
      <div className={styles.coursesTitle}>
        ...
        <p className={styles.subtitle}>Start your learning journey with our in-demand courses</p>
      </div>
        ...
        {courses.map((course) => (
          <div key={course.id} className={styles.courseCard}>
            ...
            <div className={styles.iconContainer}>
              <Image src={course.icon} alt={`${course.name} icon`} width={60} height={60} className={styles.courseIcon} />
            </div>
            ...
            <div className={isLowSeats(course.count) ? `${styles.seatsCount} ${styles.lowSeats}` : styles.seatsCount}>
              <span className={styles.seats}>{course.count} {course.count === 1 ? "Seat" : "Seats"} Left</span>
              {isLowSeats(course.count) && <span className={styles.hurryText}>Hurry up!</span>}
            </div>

            <Button className={styles.enrollBtn} onClick={() => handleEnrollNowClick(course)}>Enroll Now</Button>
          </div>
        ))}
      </div>

      <div className={styles.bottomButtons}>
        <Button className={styles.outlineBtnn} onClick={handleButtonClick}>Download Brochure</Button>
        {showForm && <Btnform onClose={handleCloseForm} />}
        <Button className={styles.outlineBtnn} onClick={() => handleEnrollNowClick({ id: "demo", name: "Free Demo Session" })}>
          Book a Free Demo
        </Button>
      </div>

      <Modal show={showPopupForm} onHide={handleClosePopupForm} centered className={styles.enrollmentModal} backdrop="static">
        <Modal.Header closeButton>
          <Modal.Title>{selectedCourse ? `Enroll in ${selectedCourse.name}` : "Enrollment Form"}</Modal.Title>
        </Modal.Header>
        <Modal.Body className={styles.modalBody}>
          <div className={styles.selectedCourseInfo}>
            <div className={styles.selectedCourseLabel}>Course:</div>
            <div className={styles.selectedCourseName}>{selectedCourse ? selectedCourse.name : ""}</div>
          </div>
